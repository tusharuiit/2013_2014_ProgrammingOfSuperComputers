...
int read_long_formatted( char *fileName, int *NINTCI, int *NINTCF, int *NEXTCI,
    ...
    for( i = 1; i <= *NBW; i++ ) {
        fscanf( fp, "%d", &( ( *NTRAEW )[i] ) );
    }
    for( i = 1; i <= *NBS; i++ ) {
        fscanf( fp, "%d", &( ( *NTRANS )[i] ) );
    }
    for( i = 1; i <= *NBL; i++ ) {
        fscanf( fp, "%d", &( ( *NTRAHL )[i] ) );
    }
...
int read_binary( char *fileName, int *NINTCI, int *NINTCF, int *NEXTCI,
    ...
    if( ( ( *LCC )[0] = ( int * ) malloc( 6 * ( *NINTCF + 1 ) * sizeof( int ) ) ) == NULL ) {
        printf( "malloc(LCC) failed\n" );
        return -1;
    }
    for( i = 1; i < 6; i++ ) {
        ( *LCC )[i] = &( *LCC )[i - 1][*NINTCF + 1];
    }
    // start reading LCC
    // Note that C array index starts from 0 while Fortran starts from 1!
    for( i = 0; i < 6; i++ ) {
        fread( ( *LCC )[i] + *NINTCI, sizeof( int ), *NINTCF - *NINTCI + 1, fp );
    }
    ...
    // read the other arrays
    fread( *BS + *NINTCI, sizeof( double ), *NINTCF - *NINTCI + 1, fp );
    fread( *BE + *NINTCI, sizeof( double ), *NINTCF - *NINTCI + 1, fp );
    fread( *BN + *NINTCI, sizeof( double ), *NINTCF - *NINTCI + 1, fp );
    fread( *BW + *NINTCI, sizeof( double ), *NINTCF - *NINTCI + 1, fp );
    fread( *BL + *NINTCI, sizeof( double ), *NINTCF - *NINTCI + 1, fp );
    fread( *BH + *NINTCI, sizeof( double ), *NINTCF - *NINTCI + 1, fp );
    fread( *BP + *NINTCI, sizeof( double ), *NINTCF - *NINTCI + 1, fp );
    fread( *SU + *NINTCI, sizeof( double ), *NINTCF - *NINTCI + 1, fp );
    ...
    fread( *NBOARD + *NINTCI, sizeof( int ), *NINTCF - *NINTCI + 1, fp );
