...
int write_result( char *inFileName, char *outFileName, int NINTCI, int NINTCF,
    ...
    int N;
    for( N = 1; N <= 10; N++ ) {
        fprintf( fp, "%lf %lf %lf %lf\n", IPOINT[N], VAR[N],   IPOINT[NINTCF - N + 1],
                 VAR[NINTCF - N + 1] );
    }
...
int write_result_vtk( char *outFileName, int startIdx, int endIdx, int nodeCnt, int **points,
                      int **elems, double *vector ) {
    int i;
    ...
    printf( "nodeCnt = %d, startInd= %d, endInd = %d, cellCnt=%d\n", nodeCnt, startIdx, endIdx,
            cellCnt );
    for( i = 1; i <= nodeCnt; i += 2 ) {
        fprintf( fp, "%d %d %d ", points[0][i], points[1][i], points[2][i] );
