...
/**
 * @brief This function is used to convert the volume-based topology (of Fire input files) to
 * unstructured mesh topology and geometry (for vtk format output).
 *
 * The volume topology is received via the input array LCC (link cell-cell).
 * One LCC[nghbId][cellId] entry in the array stores the neighboring cell of cellId in
 * direction nghbId.
...
int vol2mesh( int startIdx, int endIdx, int **LCC, int *nodeCnt, int *** points,  int *** elems ) {
     ...
     *              H - 5
     * */
    int node2nghbMap[8][6] = {  // node 7 on S nghb, node 4 on W nghb, node 5 on L nghb
        {0, 1, 3, 3, 4, 4}, {3, 2, 4, 5, 2, 0}, {4, 6, 1, 1, 2, 3},
        {0, 2, 4, 7, 1, 0}, {3, 7, 0, 5, 5, 0}, {3, 6, 5, 1, 2, 4},
        {5, 2, 2, 7, 1, 5}, {0, 6, 5, 3, 1, 4}
    };
            ...
            if( !foundVal ) {
                ...
                if( nodes == 1 ) {
                    ...
                    for( coordIdx = 0; coordIdx < 3; coordIdx++ ) {
                        ( *points )[coordIdx][nodes] = ( *points )[coordIdx][( *elems )[nodeIdx - 1][NC]] +
                                                       OFFSET[nodeIdx][coordIdx];
                    }
                }
            }
        }  // for each node
