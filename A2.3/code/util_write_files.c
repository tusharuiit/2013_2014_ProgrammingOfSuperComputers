...
void vtk_write_unstr_grid_header( const char *experiment_name, const char *out_file_name,
    ...
    /*
     * The first line notes how many points there will be and in which format they'll be supplied
     * (here "DOUBLE").
     * Each following line contains the xyz-coordinates of a point.
     * Based on this list, each point is assigned an id. The first point has id 0, the second point
     * id 1, and so forth.
     */
    ...
    /*
     * The first line notes how many cells there will be and how many numbers total will be
     * supplied in the CELLS-block.
     * Each following cell line starts with a number saying how many point IDs are to be read in
     * that line (here 8) followed by the list of those point IDs.
     * Based on this list, each cell is assigned an id. The first cell has id 0, the second id 1,
     * etc.
     */
    ...
    /*
     * The first line gives how many cell-types are to be set (= the number of cells given in
     * CELLS).
     * The following line is a list of cell-types that are assigned to all cells. The first cell is
     * of type "11", so is the second cell and so forth.
     * Cell type "11" is VTK_VOXEL (volume cell) and requires 8 point coordinates to define the  of
     * each cell (given in CELLS).
     */
    ...
    if( fclose( fp ) ) {
        fprintf( stderr, "Failed to close %s", out_file_name );
    }
}
